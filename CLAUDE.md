...
Detect-only requests get the report and no rewrite: name the constraint, quote the line, give the fix, report substance-test findings under their test name with the artifact, and close the report with a short "Not flagged" list naming the patterns deliberately not reported (quoted or secondhand text, register-appropriate for the genre, the writer's own habit, a load-bearing hedge). A closing offer to rewrite is fine; doing the rewrite unasked is not. Never flag or rewrite a watched phrase inside quotation marks, blockquotes, or titles — the mention is not the tell. When the author is a non-native English speaker, style-evidenced flags need a higher bar: suppress isolated flags, report only clusters of three or more independent signals (signals of different kinds — one construction seen three ways is one signal), and say so. Never score the draft, and never estimate whether a model wrote it — detectors guess, and a named pattern is evidence the user can verify themselves.

## Edit mode

If the user hands over their own draft to fix (rather than asking for new text), follow Phase 2.5 preservation mode in SKILL.md. The constraints still apply, but the author's voice outranks them: make the minimum effective edit, protect hedges that carry real uncertainty, keep profanity and bluntness and digressions, leave quoted material verbatim, and return the edited text plus a short "What changed" list so the user can reject an individual edit. The list ends with a claims audit ("Claims added: 0") — any fact, name, number, or date not in the source draft and not supplied by the user is an invention to remove. Phase 5's no-meta-commentary rule governs composition, not editing.
...
- **Tier 1** (C1, C9, C15, C21, C23, C24, C26, C34, C37, C43): Single match flags the piece. Always rewrite, with one carve-out: C26's wordy-construction rows ("in order to" and kin) fall under the human-signals gate in SKILL.md, which outranks the tier for those rows; C26's chatbot-tell rows stay kill-always. C15 (em dashes) and C37 (uniform hyphenation across compound modifiers) are conspicuous reader-facing structural tells in 2026; em-dash count must not exceed word_count/500. C43 (fake-profound kickers) sits in closing position and is graded hardest there — delete the kicker, never rewrite it into a better metaphor.
